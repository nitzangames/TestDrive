    ...
    shader.fragmentShader = shader.fragmentShader.replace(
      ...
      `#include <color_fragment>
       if (uStyle == 0) {
         ...
         // Cartograph: keep the biome-driven vertex color but add the WW1
         // aerial-photo "patchwork" feel as a per-cell brightness variation
         // (3 variants at ~2.5m XZ scale). The colours themselves now come
         // from the biome bands, not a hardcoded palette.
         vec3 cellPos = floor(vWorldPosT * 0.4);
         float h = fract(sin(dot(cellPos.xz, vec2(12.9898, 78.233))) * 43758.5453);
         // 3 variants: 0.85x, 1.0x, 1.15x brightness
