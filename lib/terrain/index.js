...
export function createTerrain(opts) {
  ...
  return {
    ...
    // Lerp the hemi + sun colours toward the biome at the player's
    // position. Should be called every frame with the player's world XZ.
    // The style still controls sky / fog / water (global look) while the
    // biome controls the local lighting. Lerp factor is per-frame so
    // transitions take ~1 s across a biome boundary.
