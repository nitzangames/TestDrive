...
import {
  queryRoadAt as roadQueryAt,
  roadInfluence, roadColorInfluence,
  ROAD_HALF_WIDTH, TOTAL_BAND, EARTH_PER_M,
  ASPHALT_R, ASPHALT_G, ASPHALT_B,
  ROCK_R, ROCK_G, ROCK_B,
} from '../roads/shared.js';
...
// Density window for tree placement, BEFORE per-biome heightScale multiplier.
// Tuned so forest (scale 1.0) gets trees from sea level up to ~the grass-mid
// band line. Desert (scale 0.35) shrinks to ~0-30 m which is just dune slopes;
// arctic (scale 1.5) opens up to 0-120 m but has no scatter anyway.
const TREE_PEAK_HEIGHT = 40;
const TREE_DENSITY_HALFWIDTH = 60;
...
export function buildChunkBuffers({ cx, cz, lod, seed, riverSegments, vertexGrid, biomeAt, bandsAt, villages = [], tunnels = [], roadGraph = null }) {
  ...
  // Per-biome height scale is also bilerped per vertex. The band thresholds
  // (GRASS_MID_LINE / ROCK_LINE / SNOW_LINE) are global, so without this a
  // flat desert (heightScale=0.35) never reaches the rock or snow bands —
  // its sandstone/dune-crest colours never showed. Dividing the band-lookup
  // height by the local scale lets each biome's full palette appear in
  // proportion to its own elevation range.
  const s00 = heightScaleAt(x0,              z0);
  const s10 = heightScaleAt(x0 + CHUNK_SIZE, z0);
  const s01 = heightScaleAt(x0,              z0 + CHUNK_SIZE);
  const s11 = heightScaleAt(x0 + CHUNK_SIZE, z0 + CHUNK_SIZE);
  ...
  // --- Road carve --------------------------------------------------------
  // Done HERE — after everything that determines raw heights, BEFORE
  // normals/colours — so the biome-band colour step naturally sees the
  // post-carve elevation and assigns the right band (sand/grass/rock/snow)
  // for the carved height. Per-vertex weights are retained so the asphalt
  // and exposed-rock tints can be layered on after the biome colour.
  let roadColorW = null;
  let roadCutAmt = null;
  if (roadGraph && roadGraph.edges && roadGraph.edges.length > 0) {
    roadColorW = new Float32Array(totalInterior);
    roadCutAmt = new Float32Array(totalInterior);
    for (let i = 0; i < totalInterior; i++) {
      const vx = positions[i * 3];
      const vz = positions[i * 3 + 2];
      const q = roadQueryAt(roadGraph, vx, vz);
      if (!q || q.dist > TOTAL_BAND) continue;
      const wY = roadInfluence(q.lateralOffset);
      if (wY <= 0) continue;
      const yBefore = positions[i * 3 + 1];
      const yAfter = q.roadY * wY + yBefore * (1 - wY);
      positions[i * 3 + 1] = yAfter;
      roadColorW[i] = roadColorInfluence(q.lateralOffset);
      roadCutAmt[i] = Math.min(1, Math.abs(yAfter - yBefore) / EARTH_PER_M);
    }
  }
  ...
  for (let j = 0; j < N; j++) {
    for (let i = 0; i < N; i++) {
      ...
      // boundaries and continuous across chunk edges. Because we ran the
      // road carve BEFORE this loop, cy0 is the post-carve elevation, so
      // a mountain peak cut down to road level now gets a sand / grass
      // band instead of its original snow / rock band.
      const tx = i / vertexGrid, tz = j / vertexGrid;
      bilerpBandsInto(tx, tz);
      // Bilerp the biome height scale at this vertex and normalise the
      // band-lookup height by it so each biome's palette covers its own
      // elevation range (desert: full sand→sandstone→dune-crest gradient,
      // arctic: visible rock and frosted-grass below the snow caps).
      const scale = (s00 * (1 - tx) + s10 * tx) * (1 - tz)
                  + (s01 * (1 - tx) + s11 * tx) * tz;
      const yBand = (cy0 + yJitter) / (scale > 0.01 ? scale : 1);
      writeColorForHeight(colors, p, cy0, yBand, scratchBands);

      // Layer the road's asphalt + exposed-rock tints on top of the biome
      // colour. cutAmt is high in deep cuts (= exposed rock face). wC is
      // high inside the asphalt corridor.
      if (roadColorW !== null) {
        const cut = roadCutAmt[idx];
        const wC = roadColorW[idx];
        if (cut > 0 || wC > 0) {
          let r = colors[p], g = colors[p + 1], b = colors[p + 2];
          if (cut > 0) {
            r = ROCK_R * cut + r * (1 - cut);
            g = ROCK_G * cut + g * (1 - cut);
            b = ROCK_B * cut + b * (1 - cut);
          }
          if (wC > 0) {
            r = ASPHALT_R * wC + r * (1 - wC);
            g = ASPHALT_G * wC + g * (1 - wC);
            b = ASPHALT_B * wC + b * (1 - wC);
          }
  ...
  const trees = (lod === 0 || lod === 1) ? placeTrees(cx, cz, seed, localSegs, TREE_GRID_PITCH, villages, tunnels, roadGraph) : [];
  const billboards = [];
...
function placeTrees(cx, cz, seed, localSegs, pitch, villages = [], tunnels = [], roadGraph = null) {
  const out = [];
  const x0 = cx * CHUNK_SIZE;
  const z0 = cz * CHUNK_SIZE;
  for (let j = 0; j < CHUNK_SIZE; j += pitch) {
    for (let i = 0; i < CHUNK_SIZE; i += pitch) {
      ...
      // Reject inside the road's asphalt corridor; for trees in the
      // transition band, blend the tree's Y toward roadY by the same
      // smoothstep the heightfield carve uses so the tree sits on the
      // carved surface instead of floating / sinking.
      if (roadGraph) {
        const qr = roadQueryAt(roadGraph, x, z);
        if (qr) {
          if (qr.dist <= ROAD_HALF_WIDTH) continue;
          if (qr.dist < TOTAL_BAND) {
            const wY = roadInfluence(qr.lateralOffset);
            y = qr.roadY * wY + y * (1 - wY);
          }
        }
      }
      ...
      // Density curve: peaks at TREE_PEAK_HEIGHT × biome scale. Same fix as
      // the colour bands — without the biome scale, forest's 300 m peaks
      // are far above the 16 ± 28 m density window and no trees fire.
      const biomeScale = heightScaleAt(x, z) || 1;
      const peak = TREE_PEAK_HEIGHT * biomeScale;
      const half = TREE_DENSITY_HALFWIDTH * biomeScale;
      const density = Math.max(0, 1 - Math.abs(y - peak) / half);
