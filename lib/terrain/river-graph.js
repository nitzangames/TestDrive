...
export function buildRiverGraph({ seed, gridN = 256, worldSize = 64000, threshold = 8 }) {
  // Rivers disabled — the carved-river look read as a wrinkle on the terrain
  // rather than as water. Returning an empty graph keeps the chunk pipeline
  // intact (carve/water-plane code still runs with no segments, no-op) while
  // dropping the visual artifact.
