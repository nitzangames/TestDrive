...
export function buildLoopRoad({ seed, terrainHeightFn, isOnWater }) {
  ...
  //     fall out of A*'s grid-aligned output. More passes = rounder curves
  //     but more closed-curve shrinkage; 20 passes loses ~10–15% area
  //     overall, which is fine for a 120 km loop. Curves are now wide
  //     enough that the car at MAX_SPEED can follow them with full lock.
  laplacianSmoothXZClosed(smoothed, 20);
