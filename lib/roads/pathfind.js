...
const WATER_PENALTY = 1e9;        // effectively forbidden
...
function reconstruct(cameFrom, goalIdx, idxFn, W, worldX, worldZ, terrainHeightFn, A, B) {
  ...
  // the ground (the slope-aware A* already chose a contouring path so
  // terrain-conforming Y is the correct profile).
  for (let i = 1; i < pts.length - 1; i++) pts[i].y = terrainHeightFn(pts[i].x, pts[i].z);
  return pts;
}
