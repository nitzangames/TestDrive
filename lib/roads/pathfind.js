...
export function findRoadPath(A, B, terrainHeightFn, isOnWater, opts = {}) {
  // For long endpoints the default fixed buffer (BUFFER = 1200m) is too tight
  // — A* needs room to detour around lakes / impassable ridges. Use a
  // distance-scaled buffer by default.
  const straightLine = Math.hypot(B.x - A.x, B.z - A.z);
  const buffer = opts.buffer !== undefined ? opts.buffer : Math.max(BUFFER, straightLine * 0.4);
  const maxOverage = opts.maxOverage !== undefined ? opts.maxOverage : MAX_PATH_OVERAGE;
  ...
  const maxAllowedG = straightLine * maxOverage;
