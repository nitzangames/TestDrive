...
function buildPolyline(a, b, terrainHeightFn) {
  ...
  // Light Y smoothing: one 3-tap pass to take the edge off polyline-step
  // noise, but keep the overall terrain shape so the road contours up and
  // down hills instead of floating flat over them. Then clamp each point
  // to >= terrainHeight at its own XZ so the road never tunnels through
  // ground after smoothing.
  for (const p of pts) p.y = terrainHeightFn(p.x, p.z);
  const nextY = pts.map(p => p.y);
  for (let i = 1; i < pts.length - 1; i++) {
    nextY[i] = (pts[i - 1].y + pts[i].y + pts[i + 1].y) / 3;
  }
  for (let i = 1; i < pts.length - 1; i++) {
    const groundY = terrainHeightFn(pts[i].x, pts[i].z);
    pts[i].y = Math.max(nextY[i], groundY);
  }
  return pts;
}
