...
export function buildRoadEdgeLines(THREE, graph) {
  ...
  // DoubleSide: triangle winding flips when we mirror the perpendicular
  // direction for the two sides of the road, so one side would otherwise be
  ...
  for (const side of [+1, -1]) {
    ...
    for (const edge of graph.edges) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const pz = (-tx / tl) * side;
        const p = poly[i];
        // Banking lifts +lateral and lowers -lateral. side=+1 places the
        // perpendicular along driver's right (lateral < 0 in shared.js's
        // convention), so the lateral coordinate for the vertex is
        // -side * offset.
        const bank = p.bank || 0;
        const tanBank = Math.tan(bank);
        const yInner = p.y + ROAD_OFFSET_Y + LINE_Y_OFFSET + (-side * innerOffset) * tanBank;
        const yOuter = p.y + ROAD_OFFSET_Y + LINE_Y_OFFSET + (-side * outerOffset) * tanBank;
        positions.push(p.x + px * innerOffset, yInner, p.z + pz * innerOffset);
        positions.push(p.x + px * outerOffset, yOuter, p.z + pz * outerOffset);
      }
