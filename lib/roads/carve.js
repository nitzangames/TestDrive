...
// Filter the chunk's scatter list (trees / cacti / etc.) to drop anything
// within the road's Y-influence band. Without this, scatter is placed at the
// raw terrain Y from the worker — but our carve pulls the terrain mesh
// toward roadY in a 65 m band, so trees in that band end up floating above
// the carved terrain (or sunk into it). Removing them entirely matches what
// you'd see in real life: nothing growing on the asphalt or the road
// shoulder.
export function filterChunkScatter(graph, scatterList) {
  ...
  for (let i = 0; i < scatterList.length; i++) {
    ...
    if (q && Math.abs(q.lateralOffset) <= TOTAL_BAND) continue; // skip
