...
export class HUD {
  ...
  _drawMinimap(ctx, W) {
    ...
    ctx.save();
    ctx.translate(cx, cy);
    // World motion direction at headingY=h is (sin h, cos h). On this
    // mini-map, world +Z maps to canvas DOWN (canvas Y increases downward).
    // The triangle's default tip is canvas-up (moveTo(0, -10)). To rotate
    // that "up" into the motion direction we need rotate(π + headingY).
