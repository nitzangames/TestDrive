...
export class ChaseCamera {
  ...
  update(car) {
    ...
    // Raised camera so the player looks more down on the car. Was up=3.
    const back = 7, up = 5, ahead = 4, eyeH = 1;
    ...
    const firstCall = !this._initialised;
    if (firstCall) {
      this._yaw = targetYaw;
    ...
    if (firstCall) {
      // Snap into place so the road manager streams the spawn area, not (0,0,0).
