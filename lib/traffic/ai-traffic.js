...
//
// Opposing traffic approaches at the SUM of speeds (~90 m/s closing), so we
// give it a much wider minimum spawn distance than same-direction traffic
// so the player has time to see and react.
const SPAWN_MIN_OPPOSING = 250;    // m — opposing cars never spawn closer
const SPAWN_MIN_SAMEDIR  =  60;    // m — same-direction safe minimum
const SPAWN_MAX_AHEAD    = 500;    // m
const SPAWN_BEHIND       = -150;   // m — same-direction can spawn behind
...
export class AITraffic {
  ...
  update(dt, physics) {
    ...
    //    entirely until armed and the arm-delay has elapsed.
    ...
    while (armed && this.active.length < ACTIVE_TARGET && this.pool.length > 0) {
      ...
      // Same-direction cars can spawn either ahead or behind. Opposing
      // cars only spawn well ahead (closing speeds are ~90 m/s — close
      // spawn would be unfair).
      const lo = sameDir ? SPAWN_BEHIND : SPAWN_MIN_OPPOSING;
      ...
      this.active.push({
        ...
        s: spawnS,
        targetSpeed,
        speed: targetSpeed, // current effective speed; throttled by following
        direction,
      });
    }
