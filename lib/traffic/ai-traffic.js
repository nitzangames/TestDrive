...
import { CAR_BUILDERS } from '../car/models/index.js';

// Pool of traffic-car builders. EXCLUDES 'coupe' since that's the
// player's car — we don't want clones of the player driving past.
const TRAFFIC_BUILDERS = Object.entries(CAR_BUILDERS)
  .filter(([key]) => key !== 'coupe')
  .map(([, fn]) => fn);
...
// Number of active cars kept around the player at each density preset.
// POOL_SIZE matches the heaviest preset so we never allocate more models.
const DENSITY_TARGETS = { none: 0, chill: 8, medium: 22, heavy: 36 };
const POOL_SIZE       = 36;
...
export class AITraffic {
  constructor({ THREE, scene, graph, rng = Math.random }) {
    ...
    for (let i = 0; i < POOL_SIZE; i++) {
      const build = TRAFFIC_BUILDERS[i % TRAFFIC_BUILDERS.length];
      const model = build(THREE, { bodyColor: PALETTE[i % PALETTE.length] });
    ...
    this._activeTarget = DENSITY_TARGETS.medium;
  ...
  // Switch the maximum active-car count at runtime. Accepts a preset name
  // (none / chill / medium / heavy) or a raw integer. If the new target is
  // BELOW the current active count, the excess cars are immediately
  // recycled — useful when the player drops density mid-game.
  setDensity(preset) {
    const target = typeof preset === 'number'
      ? preset
      : (DENSITY_TARGETS[preset] !== undefined ? DENSITY_TARGETS[preset] : DENSITY_TARGETS.medium);
    this._activeTarget = target;
    while (this.active.length > target) {
      const c = this.active.pop();
  ...
  update(dt, physics) {
    ...
    while (armed && this.active.length < this._activeTarget && this.pool.length > 0) {
  ...
  resolveCollisions(physics) {
    ...
    for (const c of this.active) {
      ...
      // Push out of overlap regardless of severity.
      physics.x += nx * overlap;
      physics.z += nz * overlap;

      // Decide CRASH vs BOUNCE by angle of impact. Player's velocity
      // direction is (sin h, cos h); collision normal `n` points FROM the
      // other car TO the player. A head-on hit means the player is
      // driving INTO the other car, so v_dir points OPPOSITE to n and
      // dot(v_dir, n) ≈ -1. A side-swipe has the player moving roughly
      // parallel to n's perpendicular, so dot ≈ 0.
      const vdx = Math.sin(physics.headingY);
      const vdz = Math.cos(physics.headingY);
      const headOn = Math.max(0, -(vdx * nx + vdz * nz));   // 0 = glance, 1 = head-on
      const CRASH_THRESHOLD = 0.45;

      if (headOn < CRASH_THRESHOLD) {
        // Brush / side-swipe: bounce off with a small energy loss. Don't
        // count as a crash — let the player keep driving.
        physics.speed *= 0.90;
        continue;
      }

      // Head-on or significant-angle hit. First contact is a crash:
      // kill the player's forward motion and notify. Don't fire again.
